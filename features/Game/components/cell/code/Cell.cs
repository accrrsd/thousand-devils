...
public partial class Cell : Node3D, ICell
{
  ...
  public void UpdateHighlight(bool value, bool affectHighlighted = true) {
    ...
    if (!affectHighlighted) return;
    if (value) Field.AddHighlightedCell(this);
    else Field.RemoveHighlightedCell(this);
  }

  public void ChangeHighlightedBorderColor(Color newColor = default) {
    MeshInstance3D modelInstance = _highlightBorder;
    StandardMaterial3D baseMaterial = modelInstance.GetSurfaceOverrideMaterial(0) as StandardMaterial3D ?? new StandardMaterial3D();
    baseMaterial.AlbedoColor = newColor == default ? _baseHighlightColor : newColor;
  ...
  public override void _Ready() {
    ...
    if (_highlightBorder.GetSurfaceOverrideMaterial(0) is not StandardMaterial3D material)
      throw new MyExceptions.NotExistingElemException("dont have border material");
    _baseHighlightColor = material.AlbedoColor;
    AdvancedReady();
  }
